...
router.get('/dashboard-stream', (req, res) => {
  ...
  console.log(`📡 SSE connection established: ${connectionId} for collection: ${collection}`);
  ...
  const heartbeatInterval = setInterval(() => {
    if (sseConnections.has(connectionId)) {
      try {
        ...
        console.log(`💔 Heartbeat failed for ${connectionId}, removing connection`);
...
async function broadcastDashboardUpdate(collection) {
  ...
  try {
    ...
    for (const [connectionId, connection] of sseConnections.entries()) {
      if (connection.collection === collection) {
        try {
          ...
          console.log(`💔 Failed to send update to ${connectionId}, removing connection`);
    ...
    console.log(`📊 Broadcasted dashboard update to ${sseConnections.size} connections for collection: ${collection}`);
...
function setupChangeStream(collection) {
  ...
  try {
    ...
    changeStream.on('change', (change) => {
      console.log(`🔄 MongoDB change detected in ${collection}:`, change.operationType);
      
      // Debounce updates to avoid overwhelming clients
      if (!changeStream.updateTimeout) {
        changeStream.updateTimeout = setTimeout(async () => {
          console.log(`📤 Broadcasting update for ${collection} to ${sseConnections.size} connections`);
          await broadcastDashboardUpdate(collection);
    ...
    console.log(`👁️ Change stream established for collection: ${collection}`);
